...
function Logo({ size=32 }) {
  ...
  return (
    <svg width={s} height={s} viewBox="0 0 120 120" fill="none">
      ...
      <circle cx="60" cy="60" r="42" fill="none" stroke="url(#logoRingA)" strokeWidth="5"
        strokeLinecap="round" strokeDasharray="195 264" strokeDashoffset="-20"
        transform="rotate(-90 60 60)" filter="url(#logoGlowF)"/>
      <circle cx="60" cy="60" r="33" fill="none" stroke="rgba(255,255,255,0.03)" strokeWidth="4"/>
      <circle cx="60" cy="60" r="33" fill="none" stroke="url(#logoRingB)" strokeWidth="4"
        strokeLinecap="round" strokeDasharray="145 207" strokeDashoffset="-40"
        transform="rotate(-90 60 60)" filter="url(#logoGlowF)"/>
      <path d="M60 28 L78 46 L42 46 Z" fill="rgba(255,255,255,0.06)"
        stroke="rgba(255,255,255,0.12)" strokeWidth="1" strokeLinejoin="round"/>
      <rect x="44" y="46" width="32" height="26" rx="2"
      ...
      <rect x="47" y="50" width="8" height="7" rx="1.5"
        fill="rgba(0,212,170,0.25)" stroke="rgba(0,212,170,0.4)" strokeWidth="0.8"/>
      <rect x="65" y="50" width="8" height="7" rx="1.5"
        fill="rgba(10,132,255,0.25)" stroke="rgba(10,132,255,0.4)" strokeWidth="0.8"/>
    </svg>
  );
}
...
function CropModal({ src, user, onSave, onCancel }) {
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.93)",display:"flex",
      alignItems:"center",justifyContent:"center",zIndex:300,padding:20}}>
      <div style={{background:"#111",borderRadius:24,padding:"24px 20px",maxWidth:340,
        width:"100%",border:`1px solid ${user.color}44`,textAlign:"center"}}>
        <div style={{fontSize:"0.9rem",fontWeight:700,color:user.color,marginBottom:4}}>
          ✂️ חתוך תמונה
        </div>
        ...
        <canvas ref={canvasRef} width={SIZE} height={SIZE}
          style={{borderRadius:"50%",cursor:dragging?"grabbing":"grab",
            ...
            boxShadow:`0 0 0 4px ${user.color}33, 0 20px 40px rgba(0,0,0,.6)`}}
          onPointerDown={onPointerDown} onPointerMove={onPointerMove}
          onPointerUp={onPointerUp} onPointerLeave={onPointerUp}
          onWheel={e=>{
            e.preventDefault();
            setScale(s=>Math.min(Math.max(s-e.deltaY*0.001,0.3),6));
          }}
        />
        <div style={{display:"flex",alignItems:"center",gap:10,margin:"14px 0 18px"}}>
          ...
          <span style={{fontSize:"0.72rem",color:"#555",minWidth:36}}>
            {Math.round(scale*100)}%
          </span>
        </div>
        <div style={{display:"flex",gap:8}}>
          <button onClick={handleSave}
            style={{flex:1,background:user.color,border:"none",borderRadius:12,
              color:"#000",fontFamily:"-apple-system,sans-serif",fontWeight:700,
              fontSize:"0.9rem",padding:"11px",cursor:"pointer"}}>
            ✓ שמור
          </button>
          <button onClick={onCancel}
            style={{background:"rgba(255,255,255,.06)",border:"none",borderRadius:12,
              color:"#666",fontFamily:"-apple-system,sans-serif",
              fontSize:"0.9rem",padding:"11px 16px",cursor:"pointer"}}>
            ביטול
          </button>
        </div>
      </div>
    ...
    <div style={{position:"relative",width:size,height:size,flexShrink:0,
      ...
      <div style={{width:size,height:size,borderRadius:"50%",overflow:"hidden",
        ...
        {photo
          ...
          : <span style={{fontFamily:"-apple-system,sans-serif",fontSize:size*0.38,
              fontWeight:800,color:user.color,userSelect:"none"}}>
              {user.name[0]}
            </span>
        }
      </div>
      {editable && (
        <div style={{position:"absolute",bottom:-1,left:-1,
          ...
          display:"flex",alignItems:"center",justifyContent:"center",
          fontSize:size*0.16,transition:"background .2s"}}>
          {hover?"✓":"📷"}
        </div>
      )}
    </div>
    ...
    <div style={{position:"relative",width:size,height:size}}>
      <svg width={size} height={size} style={{transform:"rotate(-90deg)"}}>
        <circle cx={size/2} cy={size/2} r={r} fill="none"
          stroke="rgba(255,255,255,.06)" strokeWidth={sw}/>
      ...
      <div style={{position:"absolute",inset:0,display:"flex",
        ...
        <span style={{fontSize:size*0.22,fontWeight:700,color:DS.text,
...
function PinModal({ user, onSuccess, onCancel }) {
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.9)",display:"flex",
      ...
      <div style={{background:DS.surface,borderRadius:24,padding:"32px 28px",
        maxWidth:320,width:"100%",border:`1px solid ${user.color}44`,textAlign:"center"}}>
        <div style={{width:56,height:56,borderRadius:"50%",background:`${user.color}18`,
          border:`2px solid ${user.color}44`,display:"flex",alignItems:"center",
          justifyContent:"center",margin:"0 auto 12px",fontFamily:"-apple-system,sans-serif",
          fontSize:"1.5rem",fontWeight:800,color:user.color}}>
          {user.name[0]}
        </div>
        <div style={{fontSize:"1.1rem",fontWeight:700,color:user.color,marginBottom:4}}>
          {user.name}
        </div>
        <div style={{color:DS.muted,fontSize:"0.82rem",marginBottom:20}}>הכנס קוד PIN</div>
        <div style={{display:"flex",gap:8,justifyContent:"center",marginBottom:16}}>
          {[0,1,2,3].map(i=>(
            <div key={i} style={{width:42,height:52,borderRadius:10,background:"#000",
              border:`2px solid ${error?"#FF453A":pin.length>i?user.color:"rgba(255,255,255,.08)"}`,
              display:"flex",alignItems:"center",justifyContent:"center",
              fontSize:"1.3rem",color:user.color,transition:"border-color .2s"}}>
              {pin.length>i?"●":""}
            </div>
          ))}
        </div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:6,marginBottom:14}}>
          {[1,2,3,4,5,6,7,8,9,"",0,"⌫"].map((k,i)=>(
            <button key={i} onClick={()=>press(String(k))} disabled={k===""}
              style={{padding:"13px 0",borderRadius:10,
                border:"1px solid rgba(255,255,255,.07)",
                background:k===""?"transparent":"rgba(255,255,255,.04)",
                color:DS.text,fontSize:"1.1rem",
                fontFamily:"-apple-system,sans-serif",
                cursor:k===""?"default":"pointer",fontWeight:600}}>
              {k}
            </button>
          ))}
        </div>
        {error && (
          <div style={{color:"#FF453A",fontSize:"0.82rem",marginBottom:10}}>קוד שגוי</div>
        )}
        <button onClick={onCancel}
          style={{background:"none",border:"none",color:DS.muted,cursor:"pointer",
...
function Toast({ msg }) {
  return (
    <div style={{position:"fixed",bottom:30,left:"50%",
      transform:`translateX(-50%) translateY(${msg?"0":"100px"})`,
      background:"#1c1c1e",color:DS.text,padding:"11px 22px",borderRadius:24,
      fontWeight:600,fontSize:"0.85rem",
      transition:"transform .35s cubic-bezier(.4,2,.6,1)",zIndex:999,
      whiteSpace:"nowrap",boxShadow:"0 8px 30px rgba(0,0,0,.5)",
      fontFamily:"-apple-system,sans-serif"}}>
      {msg||"‎"}
    </div>
  );
...
function SyncDot({ saving }) {
  return (
    <div style={{position:"fixed",top:12,left:14,zIndex:200,display:"flex",
      alignItems:"center",gap:5,fontSize:"0.68rem",color:DS.muted,
      fontFamily:"-apple-system,sans-serif"}}>
      <div style={{width:7,height:7,borderRadius:"50%",
        background:saving?"#FFD60A":DS.accent,
        boxShadow:saving?`0 0 5px #FFD60A`:`0 0 5px ${DS.accent}`,
...
function PhotoUpload({ user, chore, onDone, onSkip }) {
  ...
  const handleUpload = async () => {
    ...
    setUploading(true);
    try {
      const url = await uploadProofPhoto(file, user.id, chore.id);
      onDone(url);
    } catch(e) {
  ...
  return (
    <div style={{background:DS.surface2,borderRadius:12,padding:"14px",
      marginTop:4,border:`1px solid ${user.color}44`}}>
      <div style={{fontSize:"0.82rem",fontWeight:700,marginBottom:10,color:user.color}}>
        📷 הוכחה — {chore.name}
      </div>
      ...
      {!preview ? (
        <div onClick={()=>inputRef.current?.click()}
          style={{width:"100%",height:120,background:"rgba(255,255,255,.03)",
            border:`2px dashed ${user.color}66`,borderRadius:10,
            display:"flex",flexDirection:"column",alignItems:"center",
            justifyContent:"center",gap:8,cursor:"pointer"}}>
          <span style={{fontSize:"2rem"}}>📸</span>
          <span style={{fontSize:"0.82rem",fontWeight:700,color:user.color}}>
            צלם / בחר תמונה
          </span>
        </div>
      ) : (
        <div style={{position:"relative",marginBottom:10}}>
          ...
          }} style={{position:"absolute",top:6,left:6,background:"rgba(0,0,0,.7)",
            border:"none",borderRadius:"50%",color:"#fff",width:26,height:26,
      ...
      {error && (
        <div style={{color:"#FF453A",fontSize:"0.78rem",marginTop:6}}>{error}</div>
      )}
      <div style={{display:"flex",gap:7,marginTop:10}}>
        <button onClick={handleUpload} disabled={!preview||uploading}
          style={{flex:1,
            background:preview&&!uploading?user.color:"rgba(255,255,255,.08)",
            border:"none",borderRadius:10,
            color:preview&&!uploading?"#000":DS.muted,
            fontFamily:"-apple-system,sans-serif",fontWeight:700,
            fontSize:"0.85rem",padding:"10px",
            cursor:preview&&!uploading?"pointer":"default"}}>
          {uploading?"מעלה...":"✓ שלח הוכחה"}
        </button>
        <button onClick={onSkip}
          style={{background:"none",border:"1px solid rgba(255,255,255,.08)",
            borderRadius:10,color:DS.muted,fontFamily:"-apple-system,sans-serif",
            fontSize:"0.8rem",padding:"10px 14px",cursor:"pointer"}}>
          דלג
        </button>
      </div>
    </div>
...
function BrushPhotosModal({ user, log, avatars, onClose }) {
  ...
  const photos = [...log]
    ...
    .reverse().slice(0,12);
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.92)",zIndex:250,
      display:"flex",alignItems:"flex-end",justifyContent:"center"}}
      onClick={onClose}>
      <div onClick={e=>e.stopPropagation()}
        style={{background:"#111",borderRadius:"24px 24px 0 0",
          padding:"20px 16px 40px",width:"100%",maxWidth:500,
        ...
        <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:18,flexShrink:0}}>
          ...
          <button onClick={onClose}
            style={{marginRight:"auto",background:"rgba(255,255,255,.07)",
              border:"none",borderRadius:"50%",width:32,height:32,
              color:"#666",cursor:"pointer",fontSize:"1rem"}}>
            ✕
          </button>
        </div>
        <div style={{overflowY:"auto",flex:1}}>
          {photos.length===0 ? (
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8}}>
              {photos.map((e,i)=>(
                <div key={i} style={{position:"relative",borderRadius:12,
                  overflow:"hidden",aspectRatio:"1",background:"#1a1a1a"}}>
                  <img src={e.photoUrl} alt="צחצוח"
                    style={{width:"100%",height:"100%",objectFit:"cover"}}/>
                  <div style={{position:"absolute",bottom:0,left:0,right:0,
                    background:"linear-gradient(transparent,rgba(0,0,0,.8))",
                    padding:"8px 6px 5px",fontSize:"0.6rem",
                    color:"#ccc",fontWeight:600}}>
                    {BRUSH_NAMES[e.choreId]}<br/>
                    <span style={{color:"#666",fontWeight:400}}>
                      {new Date(e.ts).toLocaleDateString("he-IL",{day:"numeric",month:"short"})}
                    </span>
                  </div>
                </div>
              ))}
...
function BrushingSection({ user, log, onLogChore }) {
  ...
  const getStreak = (choreId) => {
    ...
    while (true) {
      ...
      const done = log.some(e =>
        e.userId===user.id &&
        e.choreId===choreId &&
        new Date(e.ts).toDateString()===dateStr
      );
      if (!done) break;
      streak++;
  ...
  return (
    <div style={{
      ...
      borderRadius:20,border:"1px solid rgba(0,212,170,0.2)",
      ...
      <div style={{
        ...
        <span style={{fontSize:"0.72rem",fontWeight:700,color:"#00D4AA",
          letterSpacing:1.5,textTransform:"uppercase"}}>
          מעקב צחצוח יומי
        </span>
        <div style={{marginRight:"auto",fontSize:"0.65rem",
          color:"rgba(0,212,170,0.5)",background:"rgba(0,212,170,0.08)",
      ...
      {/* Cards */}
      <div style={{padding:"14px 14px 4px",display:"grid",
        ...
        {BRUSH_CHORES.map(chore => {
          ...
          const lastPhoto = [...log].reverse().find(e =>
            e.userId===user.id &&
            e.choreId===chore.id &&
            e.photoUrl &&
              ...
              >
                {done && (
                  <div style={{position:"absolute",top:-20,right:-20,width:60,height:60,
                    borderRadius:"50%",background:"rgba(0,212,170,0.12)",
                ...
                <div style={{display:"flex",alignItems:"center",
                  ...
                  <div style={{width:28,height:28,borderRadius:"50%",
                    ...
                    display:"flex",alignItems:"center",justifyContent:"center",
                    fontSize:"0.85rem",flexShrink:0,transition:"all .3s"}}>
                    {done?"✓":""}
                  </div>
                </div>
                <div style={{fontSize:"0.82rem",fontWeight:700,
                  color:done?"#00D4AA":DS.text,marginBottom:4}}>
                  {chore.name}
                </div>
                <div style={{display:"flex",alignItems:"center",gap:6}}>
                  {streak > 0 && (
                    <div style={{fontSize:"0.65rem",color:"#FF9F0A",fontWeight:700,
                      background:"rgba(255,159,10,0.1)",borderRadius:8,
                      padding:"2px 7px",border:"1px solid rgba(255,159,10,0.2)"}}>
                      🔥 {streak}
                    </div>
                  )}
                  {lastPhoto && (
                    <img src={lastPhoto.photoUrl} alt="proof"
                      style={{width:22,height:22,borderRadius:6,objectFit:"cover",
                        border:"1px solid rgba(0,212,170,0.3)",marginRight:"auto"}}/>
                  )}
                  {!done && (
                    <div style={{fontSize:"0.6rem",color:DS.muted,marginRight:"auto"}}>
                      טרם בוצע
                    </div>
                  )}
                </div>
      ...
      <div style={{padding:"10px 14px 14px"}}>
        <div style={{background:"rgba(0,0,0,0.3)",borderRadius:12,
          padding:"10px 12px",border:"1px solid rgba(0,212,170,0.08)"}}>
          <div style={{fontSize:"0.6rem",color:"rgba(0,212,170,0.5)",fontWeight:600,
          ...
          <div style={{display:"flex",gap:4}}>
            {Array.from({length:7}).map((_,i) => {
              ...
              const morning = log.some(e =>
                e.userId===user.id &&
                e.choreId==="brush-morning" &&
                new Date(e.ts).toDateString()===dStr
              );
              const evening = log.some(e =>
                e.userId===user.id &&
                e.choreId==="brush-evening" &&
                new Date(e.ts).toDateString()===dStr
              );
              ...
              return (
                <div key={i} style={{flex:1,display:"flex",flexDirection:"column",
                  ...
                  <div style={{
                    ...
                    display:"flex",alignItems:"center",justifyContent:"center",
                    fontSize:"0.6rem",transition:"all .3s",
                  }}>
                    {both ? "✓✓" : morning ? "☀️" : evening ? "🌙" : ""}
                  </div>
                  <div style={{fontSize:"0.55rem",
                    color:isToday?"#00D4AA":DS.dim,
  ...
  return (
    <div style={{display:"flex",alignItems:"center",gap:10,margin:"22px 0 12px"}}>
      ...
      <div style={{display:"flex",alignItems:"center",gap:6,padding:"4px 12px",
        ...
        <span style={{fontSize:"0.65rem",fontWeight:700,color:DS.dim,
          letterSpacing:1.5,textTransform:"uppercase"}}>
          {label}
        </span>
      </div>
...
function Dashboard({ log, bonus, avatars, onSwitchUser }) {
  ...
  const sorted = [...kids].sort((a,b)=>
    getUserScore(b.id,log,bonus)-getUserScore(a.id,log,bonus)
  );
  const lastEntry = [...log].reverse().find(e=>
    !["brush-morning","brush-evening"].includes(e.choreId)
  );
  ...
  return (
    <div style={{padding:"16px 16px 80px",maxWidth:900,margin:"0 auto",
      fontFamily:"-apple-system,sans-serif",direction:"rtl",color:DS.text}}>

      {/* Header */}
      <div style={{display:"flex",justifyContent:"space-between",
        alignItems:"center",marginBottom:18}}>
        <div>
          <div style={{fontSize:"1.4rem",fontWeight:700,letterSpacing:-0.5}}>
            משימות הבית
          </div>
      ...
      {/* Brushing status */}
      <div style={{background:"linear-gradient(135deg, #071211 0%, #0d1f1b 100%)",
        borderRadius:16,border:"1px solid rgba(0,212,170,0.2)",
        padding:"12px 14px",marginBottom:14,
        boxShadow:"0 0 30px rgba(0,212,170,0.05)"}}>
        <div style={{fontSize:"0.6rem",fontWeight:800,color:"#00D4AA",
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8}}>
          {kids.map(k=>{
            ...
            const both = morning && evening;
            const lastBrush = [...log].reverse().find(e=>
              e.userId===k.id &&
              ["brush-morning","brush-evening"].includes(e.choreId)
            );
            return (
              <div key={k.id} onClick={()=>onSwitchUser(k.id)}
                style={{background:both?"rgba(0,212,170,0.1)":"rgba(0,0,0,0.3)",
                ...
                <div style={{fontSize:"0.75rem",fontWeight:700,
                  color:k.color,margin:"5px 0 2px"}}>
                  {k.name}
                </div>
                <div style={{fontSize:"0.55rem",
                  color:lastBrush?"rgba(0,212,170,0.6)":"rgba(255,255,255,0.2)",
                  marginBottom:6,fontWeight:600}}>
                  {lastBrush ? timeAgo(lastBrush.ts) : "טרם צחצח"}
                </div>
                <div style={{display:"flex",gap:3,justifyContent:"center"}}>
                  <div style={{fontSize:"0.6rem",padding:"2px 6px",borderRadius:6,
                    fontWeight:700,
                    background:morning?"rgba(0,212,170,0.2)":"rgba(255,255,255,0.04)",
                    color:morning?"#00D4AA":"rgba(255,255,255,0.2)",
                    border:`1px solid ${morning?"rgba(0,212,170,0.4)":"rgba(255,255,255,0.06)"}`}}>
                    🌅 {morning?"✓":"✗"}
                  </div>
                  <div style={{fontSize:"0.6rem",padding:"2px 6px",borderRadius:6,
                    fontWeight:700,
                    background:evening?"rgba(0,212,170,0.2)":"rgba(255,255,255,0.04)",
                    color:evening?"#00D4AA":"rgba(255,255,255,0.2)",
                    border:`1px solid ${evening?"rgba(0,212,170,0.4)":"rgba(255,255,255,0.06)"}`}}>
                    🌙 {evening?"✓":"✗"}
                  </div>
                </div>
              </div>
      ...
      {/* Last chore */}
      {lastUser && (
        <div style={{background:DS.surface,borderRadius:14,padding:"12px 14px",
          border:`1px solid ${DS.border}`,marginBottom:14,
          ...
          <div>
            ...
            <div style={{fontSize:"0.85rem",fontWeight:700,color:lastUser.color}}>
              {lastUser.name} · {lastEntry.choreTitle}
            </div>
            <div style={{fontSize:"0.65rem",color:DS.dim}}>{timeAgo(lastEntry.ts)}</div>
          </div>
          <div style={{marginRight:"auto",fontSize:"1.1rem",fontWeight:800,
      ...
      {/* Leaderboard */}
      <div style={{fontSize:"0.65rem",fontWeight:700,color:DS.dim,
        letterSpacing:1.5,marginBottom:10,textTransform:"uppercase"}}>
        LEADERBOARD
      </div>
      <div style={{background:DS.surface,borderRadius:14,
        border:`1px solid ${DS.border}`,overflow:"hidden",marginBottom:20}}>
        {sorted.map((u,i)=>{
          const sc = getUserScore(u.id,log,bonus);
          return (
            <div key={u.id}
              style={{padding:"12px 14px",
                borderBottom:i<sorted.length-1?`1px solid ${DS.border}`:"none",
                cursor:"pointer"}}
              onMouseEnter={e=>e.currentTarget.style.background="rgba(255,255,255,.03)"}
              onMouseLeave={e=>e.currentTarget.style.background="transparent"}>
              <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:5}}>
                <span style={{fontSize:"0.68rem",color:DS.dim,width:12,
                ...
                <span onClick={()=>onSwitchUser(u.id)}
                  style={{fontSize:"0.92rem",fontWeight:800,color:u.color,
              ...
              <div style={{height:3,background:"rgba(255,255,255,.05)",
                borderRadius:2,overflow:"hidden",marginRight:22}}>
                <div style={{height:"100%",
                  width:`${Math.min(
                    (sc/Math.max(...sorted.map(u=>getUserScore(u.id,log,bonus)),1))*100,
                    100
                  )}%`,
                  background:u.color,borderRadius:2,transition:"width .6s"}}/>
              </div>
            </div>
          );
        })}
      ...
      <div style={{background:DS.surface,borderRadius:14,
        border:`1px solid ${DS.border}`,padding:"0 14px"}}>
          ...
          return (
            <div key={i} style={{display:"flex",alignItems:"center",gap:10,
              ...
              <div style={{flex:1}}>
                ...
                {e.duration && (
                  <span style={{fontSize:"0.7rem",color:DS.dim}}> ⏱{e.duration}ד'</span>
                )}
              </div>
              <span style={{fontSize:"0.7rem",fontWeight:700,color:u?.color,
                fontVariantNumeric:"tabular-nums"}}>
                +{e.pts}
              </span>
              <span style={{fontSize:"0.65rem",color:DS.dim,minWidth:36,textAlign:"left"}}>
                {timeAgo(e.ts)}
              </span>
            </div>
          );
        ...
        {log.length===0 && (
          <div style={{textAlign:"center",color:DS.muted,
            padding:"32px 20px",fontSize:"0.85rem"}}>
            עדיין אין משימות 😴
          </div>
        )}
      </div>
    ...
    <div style={{marginBottom:20}}>
      ...
      <div style={{display:"flex",gap:10,overflowX:"auto",
        ...
        {rewards.map(r=>{
          ...
          return (
            <div key={r.id} style={{flexShrink:0,width:130,background:DS.surface,
              borderRadius:14,padding:12,
              border:`1px solid ${canRedeem?user.color:DS.border}`,
              ...
              <div style={{fontSize:"0.82rem",fontWeight:800,textAlign:"center",
                color:canRedeem?user.color:DS.muted,fontVariantNumeric:"tabular-nums"}}>
                {r.pts} נק'
              </div>
              {!canRedeem && (
                <div style={{height:3,background:"rgba(255,255,255,.07)",
                  ...
                  <div style={{height:"100%",width:`${pct}%`,
                    background:user.color,borderRadius:2}}/>
                </div>
              )}
              {!canRedeem && (
                <div style={{fontSize:"0.65rem",color:DS.dim,textAlign:"center"}}>
                  עוד {r.pts-score}
                </div>
              )}
              {canRedeem && (
                <button onClick={()=>onRedeemReward(r.id,user.id)}
                  style={{background:user.color,border:"none",borderRadius:8,
                    color:"#000",fontFamily:"-apple-system,sans-serif",
                    fontWeight:700,fontSize:"0.8rem",padding:"6px",cursor:"pointer"}}>
                  מימוש!
                </button>
              )}
              {redeemed > 0 && (
                <div style={{fontSize:"0.65rem",color:DS.accent,textAlign:"center"}}>
                  ✓ {redeemed}×
                </div>
              )}
            </div>
  ...
  return (
    <div style={{padding:"16px 16px 80px",maxWidth:900,margin:"0 auto",
      ...
      <input ref={inputRef} type="file" accept="image/*"
        style={{display:"none"}} onChange={e=>handleAvatarFile(e.target.files?.[0])}/>
      {cropSrc && (
        <CropModal src={cropSrc} user={user}
          onSave={url=>{onPhotoChange(url);setCropSrc(null);}}
          onCancel={()=>setCropSrc(null)}/>
      )}

      {/* Header */}
      <div style={{display:"flex",alignItems:"center",gap:14,marginBottom:18}}>
        <Avatar user={user} photo={photo} size={80} editable
          onClick={()=>inputRef.current?.click()}/>
        <div style={{flex:1}}>
          <div style={{fontSize:"1.3rem",fontWeight:700,letterSpacing:-0.3}}>
            {user.name}
          </div>
          <div style={{fontSize:"0.7rem",color:DS.muted,marginTop:1}}>{career.title}</div>
          <div style={{fontSize:"0.6rem",color:DS.dim,marginTop:2}}>
            לחץ על התמונה לשינוי
          </div>
        </div>
        <div style={{textAlign:"left"}}>
          <div style={{fontVariantNumeric:"tabular-nums",fontSize:"1.8rem",
            fontWeight:800,color:user.color,letterSpacing:-1}}>
            {score}
          </div>
      ...
      {nextReward && (
        <div style={{background:DS.surface,borderRadius:12,padding:"11px 14px",
          border:`1px solid ${DS.border}`,marginBottom:16,
          ...
          <div style={{flex:1}}>
            <div style={{display:"flex",justifyContent:"space-between",marginBottom:5}}>
              ...
              <span style={{fontSize:"0.68rem",color:DS.muted}}>
                עוד {nextReward.pts-score} נק'
              </span>
            </div>
            <div style={{height:4,background:"rgba(255,255,255,.07)",
              borderRadius:2,overflow:"hidden"}}>
              <div style={{height:"100%",
                width:`${Math.min((score/nextReward.pts)*100,100)}%`,
                background:user.color,borderRadius:2,transition:"width .6s"}}/>
            </div>
            <div style={{display:"flex",justifyContent:"space-between",marginTop:3}}>
              <span style={{fontSize:"0.62rem",color:user.color,fontWeight:700}}>
                {score}
              </span>
      ...
      <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:16}}>
        {BADGES.map(b=>{
          const earned = b.cond(userStats);
          return (
            <div key={b.id} style={{background:DS.surface,
              border:`1px solid ${earned?user.color:DS.border}`,
              borderRadius:14,padding:"4px 10px",fontSize:"0.7rem",
              fontWeight:600,color:earned?user.color:DS.dim}}>
              {b.icon} {b.title}
            </div>
          );
        })}
      ...
      {/* Rewards */}
      <KidRewards user={user} rewards={rewards||[]} log={log}
        bonus={bonus} onRedeemReward={onRedeemReward}/>
      ...
      {CHORE_CATEGORIES.map(cat=>{
        ...
        return (
          <div key={cat.id} style={{marginBottom:7}}>
              ...
              <div style={{display:"flex",gap:2}}>
                {cat.chores.slice(0,5).map((_,i)=>(
                  <div key={i} style={{width:5,height:5,borderRadius:"50%",
                    background:i<doneInCat?user.color:"rgba(255,255,255,.1)"}}/>
                ))}
              </div>
              <span style={{fontSize:"0.62rem",
                color:doneInCat>0?user.color:DS.muted}}>
                {doneInCat}/{cat.chores.length}
              </span>
              <span style={{color:DS.dim,fontSize:"0.65rem",
            ...
            {isOpen && (
              <div style={{background:DS.surface,borderRadius:"0 0 12px 12px",
                border:`1px solid ${DS.border}`,borderTop:"none",padding:"3px 0"}}>
                {cat.chores.map((chore,ci)=>{
                  const timesToday = countToday(user.id,chore.id,log);
                  const lastLog = [...log].reverse().find(e=>
                    e.userId===user.id && e.choreId===chore.id
                  );
                  const isActive = activeChore?.choreId===chore.id;
                  return (
                    <div key={chore.id}>
                      <div onClick={()=>openTimeLog(chore,cat.id)}
                        style={{display:"flex",alignItems:"center",gap:10,
                          padding:"9px 14px",
                          borderBottom:ci<cat.chores.length-1
                        ...
                        onMouseLeave={e=>
                          e.currentTarget.style.background=
                            isActive?"rgba(255,255,255,.04)":"transparent"}>
                        <div style={{width:22,height:22,borderRadius:7,
                          border:`1.5px solid ${timesToday>0?user.color:"rgba(255,255,255,.12)"}`,
                          background:timesToday>0?user.color:"transparent",
                          display:"flex",alignItems:"center",justifyContent:"center",
                          flexShrink:0,fontSize:"0.62rem",fontWeight:800,color:"#000"}}>
                          {timesToday>0?timesToday:""}
                        </div>
                        <span style={{flex:1,fontSize:"0.82rem"}}>{chore.name}</span>
                        {lastLog?.duration && (
                          <span style={{fontSize:"0.65rem",color:DS.dim}}>
                            ⏱{lastLog.duration}ד'
                          </span>
                        )}
                        <span style={{fontSize:"0.68rem",fontWeight:700,
                          color:user.color,fontVariantNumeric:"tabular-nums"}}>
                          +{chore.pts}
                        </span>
                      </div>
                      {isActive && (
                        <div style={{background:DS.surface2,padding:"12px 14px",
                          borderBottom:ci<cat.chores.length-1
                            ?`1px solid ${DS.border}`:"none"}}>
                          <div style={{fontSize:"0.78rem",color:DS.muted,marginBottom:8}}>
                            ⏱ כמה זמן לקח לך?
                          </div>
                          <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:8}}>
                            {[2,5,10,15,20,30].map(m=>(
                              <button key={m} onClick={()=>setSelectedTime(m)}
                                style={{padding:"5px 11px",borderRadius:18,
                                  background:selectedTime===m
                                    ?user.color:"rgba(255,255,255,.05)",
                                  border:`1px solid ${selectedTime===m
                                    ?user.color:"rgba(255,255,255,.08)"}`,
                                  color:selectedTime===m?"#000":DS.text,
                                  fontFamily:"-apple-system,sans-serif",
                          ...
                          <div style={{display:"flex",gap:6,marginBottom:8}}>
                            <input type="number" placeholder="זמן אחר"
                              value={customTime}
                              onChange={e=>setCustomTime(e.target.value)}
                              style={{flex:1,background:"rgba(255,255,255,.04)",
                                border:"1px solid rgba(255,255,255,.08)",
                                borderRadius:8,color:DS.text,padding:"6px 10px",
                                fontFamily:"-apple-system,sans-serif",fontSize:"0.82rem"}}/>
                            <button
                              onClick={()=>confirm(customTime?parseInt(customTime):selectedTime)}
                              style={{background:user.color,border:"none",borderRadius:8,
                                color:"#000",fontFamily:"-apple-system,sans-serif",
                          ...
                          <button onClick={()=>confirm(null)}
                            style={{background:"none",
                              border:"1px solid rgba(255,255,255,.07)",
                              borderRadius:18,color:DS.muted,
                              fontFamily:"-apple-system,sans-serif",
                              fontSize:"0.75rem",padding:"4px 12px",cursor:"pointer"}}>
                            דלג
                          </button>
                        </div>
                      )}
        ...
        {[...ulog].reverse().slice(0,8).map((e,i,arr)=>(
          <div key={i} style={{display:"flex",alignItems:"center",gap:10,
            ...
            <div style={{flex:1}}>
              {e.choreTitle}
              {e.duration && (
                <span style={{fontSize:"0.68rem",color:DS.dim}}>
                  {" "}⏱{e.duration}ד'
                </span>
              )}
              {e.photoUrl && (
                <a href={e.photoUrl} target="_blank" rel="noreferrer"
                  ...
                  <img src={e.photoUrl} alt="הוכחה"
                    style={{width:26,height:26,borderRadius:5,objectFit:"cover",
                      border:"1px solid rgba(255,255,255,.1)"}}/>
                </a>
              )}
            </div>
            <span style={{fontSize:"0.75rem",fontWeight:700,color:user.color,
              fontVariantNumeric:"tabular-nums"}}>
              +{e.pts}
            </span>
...
// ─── PARENT PAGE ──────────────────────────────────────────────────────────────
function ParentPage({
  user, log, bonus, onAdjustBonus, onSwitchUser,
  onEditEntry, onDeleteEntry, rewards, onAddReward,
  onDeleteReward, onRedeemReward, avatars
}) {
  const kids = USERS.filter(u=>u.role==="kid");
    ...
    <div style={{padding:"16px 16px 80px",maxWidth:900,margin:"0 auto",
      ...
      <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:20}}>
        <div style={{width:48,height:48,borderRadius:"50%",
          background:`${user.color}15`,border:`1.5px solid ${user.color}44`,
          display:"flex",alignItems:"center",justifyContent:"center",
          fontSize:"1.3rem",fontWeight:800,color:user.color}}>
          {user.name[0]}
        </div>
        <div>
      ...
      {/* Brushing overview */}
      <div style={{background:"linear-gradient(135deg, #071211, #0a1a16)",
        borderRadius:16,border:"1px solid rgba(0,212,170,0.2)",
        padding:"14px",marginBottom:20}}>
        <div style={{fontSize:"0.65rem",fontWeight:700,color:"#00D4AA",
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8}}>
          {kids.map(k => {
            ...
            return (
              <div key={k.id} style={{background:"rgba(0,0,0,0.3)",borderRadius:12,
                padding:"10px 8px",textAlign:"center",
                border:`1px solid ${k.color}22`}}>
                <Avatar user={k} photo={avatars?.[k.id]} size={32}/>
                <div style={{fontSize:"0.75rem",fontWeight:700,
                  color:k.color,marginTop:4,marginBottom:6}}>
                  {k.name}
                </div>
                <div style={{display:"flex",gap:4,justifyContent:"center"}}>
                  <div style={{fontSize:"0.6rem",padding:"2px 6px",borderRadius:6,
                    background:morning?"rgba(0,212,170,0.2)":"rgba(255,255,255,0.04)",
                    color:morning?"#00D4AA":DS.dim,
                    border:`1px solid ${morning?"rgba(0,212,170,0.4)":"rgba(255,255,255,0.06)"}`}}>
                    🌅{morning?"✓":"✗"}
                  </div>
                  <div style={{fontSize:"0.6rem",padding:"2px 6px",borderRadius:6,
                    background:evening?"rgba(0,212,170,0.2)":"rgba(255,255,255,0.04)",
                    color:evening?"#00D4AA":DS.dim,
                    border:`1px solid ${evening?"rgba(0,212,170,0.4)":"rgba(255,255,255,0.06)"}`}}>
                    🌙{evening?"✓":"✗"}
                  </div>
                </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",
        ...
        {kids.map(k=>{
          ...
          const wk = weekLog.filter(e=>e.userId===k.id).length;
          return (
            <div key={k.id} onClick={()=>onSwitchUser(k.id)}
              style={{background:DS.surface,borderRadius:14,padding:"14px 10px",
              ...
              <div style={{fontSize:"0.82rem",fontWeight:700,color:k.color,marginTop:6}}>
                {k.name}
              </div>
              <div style={{fontSize:"1.3rem",fontWeight:800,color:k.color,
                fontVariantNumeric:"tabular-nums"}}>
                {sc}
              </div>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:20}}>
        {kids.map(k=>(
          <div key={k.id} style={{background:DS.surface,borderRadius:12,
            padding:"12px 14px",border:`1px solid ${DS.border}`,
            display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
            <Avatar user={k} photo={avatars?.[k.id]} size={30}/>
            <span style={{fontWeight:600,color:k.color,flex:1,fontSize:"0.85rem"}}>
              {k.name}
            </span>
            {[-10,-5,-1].map(n=>(
              <button key={n} onClick={()=>onAdjustBonus(k.id,n)}
            ...
            <span style={{fontSize:"1.1rem",fontWeight:800,color:k.color,
        ...
        return (
          <div key={k.id} style={{marginBottom:16}}>
            <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:8}}>
              ...
              <span style={{fontSize:"0.82rem",fontWeight:700,color:k.color}}>
                {k.name} · {sc} נקודות
              </span>
            </div>
            <div style={{display:"grid",
              ...
              {CAREERS.map(c=>{
                const unlocked = sc >= c.pts;
                return (
                  <div key={c.pts} style={{background:DS.surface,borderRadius:10,
                    padding:"10px 8px",textAlign:"center",
                    border:`1px solid ${unlocked?k.color:DS.border}`,
                    opacity:unlocked?1:.35}}>
                    <div style={{fontSize:"0.72rem",fontWeight:700,
                      color:unlocked?k.color:DS.muted}}>
                      {c.title}
                    </div>
                    <div style={{fontSize:"0.6rem",color:DS.dim,marginTop:2}}>
                      {c.pts===0?"התחלה":c.pts+" נק'"}{unlocked?" ✓":""}
                    </div>
                  </div>
                );
              })}
      ...
      {(rewards||[]).length>0 && (
        <div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:10}}>
          {(rewards||[]).map(r=>{
            ...
            return (
              <div key={r.id} style={{background:DS.surface,borderRadius:12,
                padding:"11px 14px",border:`1px solid ${DS.border}`,
                display:"flex",alignItems:"center",gap:10,flexWrap:"wrap"}}>
                <span style={{fontSize:"1.3rem"}}>{r.emoji||"🎁"}</span>
                <div style={{flex:1}}>
                  <div style={{fontWeight:700,fontSize:"0.85rem"}}>{r.title}</div>
                  <div style={{fontSize:"0.7rem",color:DS.muted}}>
                    {r.pts} נקודות · מומש {totalRedeemed}×
                  </div>
                </div>
                <div style={{display:"flex",gap:4}}>
                  {kids.map(k=>{
                    ...
                    return (
                      <button key={k.id}
                        onClick={()=>can&&onRedeemReward(r.id,k.id)}
                        title={can
                          ?`מימוש עבור ${k.name}`
                          :`${k.name} חסר ${r.pts-sc} נק'`}
                        style={{width:28,height:28,borderRadius:"50%",
                          border:`1.5px solid ${can?k.color:"rgba(255,255,255,.08)"}`,
                          background:"transparent",
                          cursor:can?"pointer":"default",
                          opacity:can?1:0.3,overflow:"hidden",padding:0}}>
                        {avatars?.[k.id]
                          ? <img src={avatars[k.id]}
                              style={{width:"100%",height:"100%",objectFit:"cover"}}/>
                          : <span style={{fontSize:"0.72rem",fontWeight:800,color:k.color}}>
                              {k.name[0]}
                            </span>
                        }
                      </button>
                ...
                <button onClick={()=>onDeleteReward(r.id)}
                  style={{background:"rgba(255,69,58,.1)",
                    border:"1px solid rgba(255,69,58,.2)",borderRadius:7,
                    color:"#FF453A",padding:"4px 9px",cursor:"pointer",
                    fontFamily:"-apple-system,sans-serif",fontSize:"0.75rem"}}>
                  🗑️
                </button>
              </div>
            );
      ...
      {!addingReward
        ? (
          <button onClick={()=>setAddingReward(true)}
            style={{display:"flex",alignItems:"center",gap:8,
              background:"rgba(255,255,255,.03)",
              border:"1px dashed rgba(255,255,255,.1)",
              borderRadius:12,padding:"11px 14px",cursor:"pointer",
              color:DS.muted,fontFamily:"-apple-system,sans-serif",
              fontSize:"0.85rem",width:"100%",marginBottom:20}}>
            ＋ הוסף פרס חדש
          </button>
        ) : (
          <div style={{background:DS.surface,borderRadius:12,padding:14,
            border:`1px solid ${user.color}44`,marginBottom:20}}>
            <div style={{fontWeight:700,marginBottom:10,color:user.color,fontSize:"0.9rem"}}>
              פרס חדש
            </div>
            <div style={{display:"flex",gap:5,flexWrap:"wrap",marginBottom:10}}>
              {REWARD_EMOJIS.map(e=>(
                <button key={e} onClick={()=>setNewReward(p=>({...p,emoji:e}))}
                  style={{width:34,height:34,borderRadius:8,
                    border:`1.5px solid ${newReward.emoji===e?user.color:"rgba(255,255,255,.08)"}`,
                    background:newReward.emoji===e?`${user.color}22`:"transparent",
                    fontSize:"1.1rem",cursor:"pointer"}}>
                  {e}
                </button>
              ))}
            </div>
            <input placeholder="שם הפרס" value={newReward.title}
              ...
              style={{width:"100%",background:"rgba(255,255,255,.05)",
                ...
                color:DS.text,padding:"9px 12px",
                fontFamily:"-apple-system,sans-serif",
                fontSize:"0.85rem",marginBottom:8,direction:"rtl"}}/>
            <input type="number" placeholder="כמה נקודות?"
              value={newReward.pts}
              ...
              style={{width:"100%",background:"rgba(255,255,255,.05)",
                ...
                color:DS.text,padding:"9px 12px",
                fontFamily:"-apple-system,sans-serif",
                fontSize:"0.85rem",marginBottom:10,direction:"rtl"}}/>
            <div style={{display:"flex",gap:7}}>
              <button onClick={()=>{
                if(!newReward.title||!newReward.pts) return;
                onAddReward({
                  title:newReward.title,
                  pts:parseInt(newReward.pts),
                  emoji:newReward.emoji
                });
                setNewReward({title:"",pts:"",emoji:"🎁"});
                setAddingReward(false);
              }} style={{flex:1,background:user.color,border:"none",borderRadius:9,
                color:"#000",fontFamily:"-apple-system,sans-serif",
                fontWeight:700,fontSize:"0.85rem",padding:"9px",cursor:"pointer"}}>
                ✓ שמור
              </button>
              <button onClick={()=>setAddingReward(false)}
                style={{background:"rgba(255,255,255,.05)",border:"none",borderRadius:9,
                  color:DS.muted,fontFamily:"-apple-system,sans-serif",
                  fontSize:"0.85rem",padding:"9px 14px",cursor:"pointer"}}>
                ביטול
              </button>
            </div>
          </div>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:7}}>
        {[...log].map((e,origIdx)=>({...e,origIdx})).reverse().map(({origIdx,...e})=>{
          ...
          return (
            <div key={origIdx} style={{background:DS.surface,borderRadius:12,
              padding:"11px 14px",
              border:`1px solid ${isEditing?user.color:DS.border}`,
              fontSize:"0.82rem"}}>
              <div style={{display:"flex",alignItems:"center",gap:8}}>
                ...
                <div style={{flex:1}}>
                  ...
                  <span>{e.choreTitle}</span>
                  {e.duration && (
                    <span style={{fontSize:"0.7rem",color:DS.dim}}>
                      {" "}⏱{e.duration}ד'
                    </span>
                  )}
                  {e.photoUrl && (
                  ...
                  <div style={{fontSize:"0.65rem",color:DS.dim,marginTop:1}}>
                    {timeAgo(e.ts)}
                  </div>
                </div>
                <span style={{fontSize:"0.85rem",fontWeight:800,color:u?.color,
                  fontVariantNumeric:"tabular-nums"}}>
                  +{e.pts}
                </span>
                <button onClick={()=>setEditingIdx(isEditing?null:origIdx)}
                  style={{background:isEditing
                    ?"rgba(255,69,58,.12)":"rgba(255,255,255,.05)",
                    border:"none",borderRadius:7,
                    color:isEditing?"#FF453A":DS.muted,
              ...
              {isEditing && (
                <div style={{marginTop:10,paddingTop:10,
                  ...
                  <div style={{display:"flex",alignItems:"center",gap:6,flexWrap:"wrap"}}>
                    <span style={{fontSize:"0.75rem",color:DS.muted,minWidth:60}}>
                      נקודות:
                    </span>
                    {[-5,-2,-1,1,2,5].map(n=>(
                      <button key={n}
                        onClick={()=>onEditEntry(origIdx,"pts",e.pts+n)}
                        style={{padding:"3px 9px",borderRadius:7,border:"none",
                          background:n>0?"rgba(48,209,88,.12)":"rgba(255,69,58,.12)",
                          color:n>0?"#30D158":"#FF453A",
                          fontFamily:"-apple-system,sans-serif",
                          fontWeight:700,fontSize:"0.78rem",cursor:"pointer"}}>
                        {n>0?"+":""}{n}
                      </button>
                    ))}
                    <span style={{fontSize:"1rem",fontWeight:800,color:u?.color,
                      marginRight:"auto",fontVariantNumeric:"tabular-nums"}}>
                      {e.pts}
                    </span>
                  </div>
                  <div style={{display:"flex",alignItems:"center",gap:6,flexWrap:"wrap"}}>
                    ...
                    {[2,5,10,15,20,30].map(m=>(
                      <button key={m}
                        onClick={()=>onEditEntry(origIdx,"duration",m)}
                        style={{padding:"3px 9px",borderRadius:7,
                          border:`1px solid ${e.duration===m
                            ?u?.color:"rgba(255,255,255,.08)"}`,
                          background:e.duration===m?`${u?.color}22`:"transparent",
                          color:e.duration===m?u?.color:DS.muted,
                          fontFamily:"-apple-system,sans-serif",
                          fontSize:"0.75rem",cursor:"pointer"}}>
                        {m}
                      </button>
                    ))}
                    <button onClick={()=>onEditEntry(origIdx,"duration",null)}
                      style={{padding:"3px 9px",borderRadius:7,
                        border:"1px solid rgba(255,255,255,.07)",
                        background:"transparent",color:DS.dim,
                        fontFamily:"-apple-system,sans-serif",
                        fontSize:"0.75rem",cursor:"pointer"}}>
                      ללא
                    </button>
                  </div>
                  <button onClick={()=>{onDeleteEntry(origIdx);setEditingIdx(null);}}
                    style={{alignSelf:"flex-start",
                      background:"rgba(255,69,58,.1)",
                      border:"1px solid rgba(255,69,58,.2)",borderRadius:8,
                      color:"#FF453A",padding:"5px 12px",cursor:"pointer",
                      fontFamily:"-apple-system,sans-serif",
...
export default function App() {
  ...
  useEffect(()=>{
    const stop = subscribeToRealtime(data=>{
      if(
        data.log?.length!==stateRef.current.log?.length ||
  ...
  const persist = useCallback((nextLog,nextBonus,nextRewards,nextAvatars)=>{
    ...
    saveTimer.current = setTimeout(async()=>{
      try {
        await saveData({log:nextLog,bonus:nextBonus,rewards:r,avatars:av});
      } catch(e) {
  ...
  const handleLogChore = (uid,choreId,catId,duration,photoUrl) => {
    ...
    if(!chore) return;
    const entry = {
      userId:uid, choreId, catId,
      choreTitle:chore.name, pts:chore.pts,
    ...
    showToast(
      `${entry.photoUrl?"📷":"✅"} ${chore.name}${chore.tracking?"":" · +"+chore.pts+" נקודות"}!`
    );
    pop();
  };
  ...
  const handleAdjustBonus = (uid,amount) => {
    ...
    showToast(
      `${amount>0?"+":""}${amount} נקודות ל${USERS.find(u=>u.id===uid)?.name}`
    );
  };
  ...
  const handleAddReward = reward => {
    const next=[
      ...
      {...reward,id:Date.now().toString(),redeemedBy:[]}
    ];
    setRewards(next);
  ...
  const handleRedeemReward = (rid,uid) => {
    const next=(stateRef.current.rewards||[]).map(r=>
      r.id===rid
        ? {...r,redeemedBy:[...(r.redeemedBy||[]),{uid,ts:Date.now()}]}
        : r
    );
    setRewards(next);
    persist(stateRef.current.log,stateRef.current.bonus,next);
  ...
  if(loading) return (
    <div style={{height:"100vh",display:"flex",alignItems:"center",
      justifyContent:"center",background:"#000",color:DS.muted,
      fontFamily:"-apple-system,sans-serif",flexDirection:"column",gap:14}}>
      <Logo size={56}/>
  ...
  return (
    <div style={{background:DS.bg,minHeight:"100vh",color:DS.text,
      fontFamily:"-apple-system,sans-serif",direction:"rtl"}}>
      <style>{`
      ...
      <nav style={{position:"sticky",top:0,zIndex:100,
        ...
        <button onClick={()=>switchUser("dash")}
          style={{flexShrink:0,padding:"13px 4px 13px 0",background:"none",
            border:"none",
            borderBottom:`2px solid ${currentUser==="dash"?DS.accent:"transparent"}`,
            cursor:"pointer",display:"flex",alignItems:"center",gap:7,
            transition:"all .2s",marginLeft:16}}>
          <Logo size={20}/>
          <span style={{fontSize:"0.82rem",fontWeight:600,
            color:currentUser==="dash"?DS.accent:DS.muted}}>
            הבית
          </span>
        </button>
        {USERS.map(u=>(
          <button key={u.id} onClick={()=>switchUser(u.id)}
            style={{flexShrink:0,padding:"13px 14px",background:"none",border:"none",
              borderBottom:`2px solid ${currentUser===u.id?u.color:"transparent"}`,
              color:currentUser===u.id?u.color:DS.muted,
              fontFamily:"-apple-system,sans-serif",fontSize:"0.82rem",fontWeight:600,
              cursor:"pointer",whiteSpace:"nowrap",transition:"all .2s",
              display:"flex",alignItems:"center",gap:7}}>
            {avatars?.[u.id]&&u.role==="kid"
              ? <img src={avatars[u.id]}
                  style={{width:18,height:18,borderRadius:"50%",objectFit:"cover",
                    border:`1px solid ${u.color}44`}}/>
              : <div style={{width:18,height:18,borderRadius:"50%",
                  background:`${u.color}18`,border:`1px solid ${u.color}44`,
                  display:"flex",alignItems:"center",justifyContent:"center",
                  fontSize:"0.6rem",fontWeight:800,color:u.color}}>
                  {u.name[0]}
                </div>
            }
            {u.name}
      ...
      {currentUser==="dash" && (
        <Dashboard
          log={log} bonus={bonus} avatars={avatars}
          onSwitchUser={switchUser}/>
      )}
      {user?.role==="kid" && (
        <KidPage
          user={user} log={log} bonus={bonus}
          onLogChore={handleLogChore} rewards={rewards}
          onRedeemReward={handleRedeemReward}
          photo={avatars[user.id]}
          onPhotoChange={url=>handleAvatarChange(user.id,url)}/>
      )}
      {user?.role==="parent" && (
        <ParentPage
          user={user} log={log} bonus={bonus}
          onAdjustBonus={handleAdjustBonus}
          onSwitchUser={switchUser}
